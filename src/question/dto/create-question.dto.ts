import { ApiProperty } from '@nestjs/swagger';

export class CreateQuestionDto {
  @ApiProperty({
    description: '문제 사진 파일의 Base64 인코딩입니다.',
    type: String,
    default:
      '',
  })
  imageBase64: string;

  @ApiProperty({
    description: '문제 사진 파일의 확장자입니다. (e.g. png, jpg)',
    type: String,
    example: 'png',
  })
  imageFormat: string;

  @ApiProperty({
    description: '학생의 질문 내용입니다.',
    type: String,
    example: '공식 유도가 안돼요 ㅠㅠ',
  })
  description: string;

  @ApiProperty({
    description: '문제의 학교 단계입니다. (e.g. 중학교, 고등학교)',
    type: String,
    example: 'HighSchool',
  })
  schoolLevel: string;

  @ApiProperty({
    description:
      '문제의 학교 과목입니다. (e.g. 수학1, 수학2, 미적분, 확률과통계)',
    type: String,
    example: '수학1',
  })
  schoolSubject: string;

  @ApiProperty({
    description: '문제의 학교 단원입니다. (e.g. 지수로그, 삼각함수, 수열)',
    type: String,
    example: '지수로그',
  })
  schoolChapter: string;

  @ApiProperty({
    description: '문제의 난이도입니다. (e.g. 쉬움, 중간, 어려움)',
    type: String,
    example: '쉬움',
  })
  difficulty: string;

  @ApiProperty({
    description: '희망하는 과외 시각입니다. (e.g. 월요일 18시, 화요일 19시)',
    type: Array,
  })
  hopeTutorialTime: string[];
}
